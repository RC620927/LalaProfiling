...
public class BezierCurveMovement implements Movement {
    ...
    public BezierCurveMovement(Point2D initialPoint, Point2D endPoint,  double initialAngle,
                               double endingAngle, double fudge1, double fudge2, int detail) {
        this.initialPoint = initialPoint;
        this.endPoint = endPoint;
        this.initialAngle = initialAngle;
        this.endingAngle = endingAngle;
        this.detail = detail;
        controlPoint1 = new Point2D.Double(initialPoint.getX() + Math.cos(Math.toRadians(initialAngle)) * fudge1,
                initialPoint.getY() + Math.sin(Math.toRadians(initialAngle)) * fudge1);
        controlPoint2 = new Point2D.Double(endPoint.getX() + Math.cos(Math.toRadians(endingAngle)) * fudge2,
                endPoint.getY() + Math.sin(Math.toRadians(endingAngle)) * fudge2);
        update();
